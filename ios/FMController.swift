        ...
        if ScanConfigs.accuraConfigs.index(forKey: "with_face") != nil {
            ...
            if gl.withFace {
                if ScanConfigs.accuraConfigs.index(forKey: "face_uri") != nil {
                    if let face = AccuraKyc.getImageFromUri(path: ScanConfigs.accuraConfigs["face_uri"] as! String) {
                        gl.face1 = face
    ...
    func facematchData(_ FaceImage: UIImage!) {
        ...
        if gl.face1 == nil {
            ...
            if gl.face1Detect == nil {
                if let img1 = AccuraKyc.getImageUri(img: gl.face1!, name: nil) {
                    results["img_1"] = img1
                }
            } else {
                if let img1 = AccuraKyc.getImageUri(img: AccuraKyc.resizeImage(image: gl.face1!, targetSize: gl.face1Detect!.bound), name: nil) {
                    results["img_1"] = img1
                }
            }
            ...
            if !gl.withFace {
                ...
                if let img2 = AccuraKyc.getImageUri(img: gl.face2!, name: nil) {
                    results["img_2"] = img2
                }
            } else {
                if let img1 = AccuraKyc.getImageUri(img: AccuraKyc.resizeImage(image: gl.face2!, targetSize: gl.face2Detect!.bound), name: nil) {
                    results["detect"] = img1
                }
            }

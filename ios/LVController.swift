    ...
    func livenessData(_ stLivenessValue: String!, livenessImage: UIImage!, status: Bool, imagePath: String!) {
        ...
        if status == true {
            ...
            if gl.face2Detect != nil {
                results["detect"] = AccuraKyc.getImageUri(img: AccuraKyc.resizeImage(image: livenessImage, targetSize: gl.face2Detect!.bound), name: nil)
            } else {
                results["detect"] = AccuraKyc.getImageUri(img: livenessImage, name: nil)
            }
            if imagePath != "" {

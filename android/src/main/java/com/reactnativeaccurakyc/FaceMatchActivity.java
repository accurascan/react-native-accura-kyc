package com.reactnativeaccurakyc;
...
public class FaceMatchActivity extends AppCompatActivity implements FaceHelper.FaceMatchCallBack, FaceCallback {
        ...
        if (bundle.containsKey("with_face")) {
            ...
            if (witFace) {
                ...
                if (base64Data.length() > 0) {
                    Bitmap face = AccuraKycModule.getBase64ToBitmap(base64Data);
                    face1 = face;
                    faceHelper.setInputImage(face);
                }
                ...
                if (!isLiveness) {
                    if (!bundle.containsKey("face1")) {
                        AccuraKycModule.faceCL.invoke("Missing face1 configuration", null);
                        this.finish();
                        return;
                    }
                    if (bundle.containsKey("face2")) {
                        ...
                        if (isFace2) {
                            if (AccuraKycModule.face1 == null) {
                              AccuraKycModule.faceCL.invoke("Please first take Face1 photo", null);
                                this.finish();
                                return;
                            } else {
                                face1 = AccuraKycModule.face1;
                                faceHelper.setInputImage(face1);
                            }
                        }
                    } else {
                      AccuraKycModule.faceCL.invoke("Missing face2 configuration", null);
                        this.finish();
                        return;
                    }
                }
          ...
          AccuraKycModule.faceCL.invoke("Missing with_face configuration", null);
            this.finish();
            return;
        }
        try {
    ...
    public void opneLiveness() throws JSONException {
        ...
        Log.i(TAG, "feedBackLowLightMessage HAS:- " + bundle.containsKey("feedBackLowLightMessage"));
...
//        }
//        if (livenessCustomization.isRecordVideo && AccuraKycModule.isLivenessGetVideo) {
//            File vid = new File(AccuraKycModule.livenessVideo);
//            if (vid.exists()) {
...
//        cameraScreenCustomization.setGlarePercentage(bundle.getInt("setGlarePercentage_0", getResources().getInteger(R("setGlarePercentage_0", "integer"))), bundle.getInt("setGlarePercentage_1", getResources().getInteger(R("setGlarePercentage_1", "integer"))));
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 202) {
            if (data == null) {
              AccuraKycModule.faceCL.invoke("No data", null);
                this.finish();
                return;
            }
                ...
                if (result.getStatus().equals("1")) {
                    livenessResult = handleVerificationSuccessResult(result, results);
                     if (witFace) {
                       ...
                       AccuraKycModule.faceCL.invoke(null, livenessResult.toString());
                         this.finish();
                     }

                } else {
                  AccuraKycModule.faceCL.invoke(result.getErrorMessage(), null);
                    if (result.getVideoPath() != null) {
                      AccuraKycModule.isLivenessGetVideo = true;
                      AccuraKycModule.livenessVideo = result.getVideoPath().getPath();
            ...
            if (result.getLivenessResult() != null) {
                if (result.getLivenessResult().getLivenessStatus()) {
                    try {
                        ...
                        if (result.getFaceBiometrics() != null) {
                            caResults.put("detect", AccuraKycModule.getImageUri(result.getFaceBiometrics(), "live_detect", getFilesDir().getAbsolutePath()));
                        }
    ...
    public void handleVerificationSuccessResultFM(final AccuraVerificationResult result) {
        if (result != null) {
            if (face1 == null) {
                if (result.getFaceBiometrics() != null) {
                    AccuraKycModule.face1 = face1 = result.getFaceBiometrics();
                    JSONObject results = new JSONObject();
                    try {
                        results.put("status", false);
                        ...
                        if (detectFace1 == null) {
                            results.put("img_1", AccuraKycModule.getImageUri(AccuraKycModule.face1, "img_1", fileDir));
                        } else {
                            results.put("img_1", AccuraKycModule.getImageUri(detectFace1, "img_1", fileDir));
                        }
                  ...
                  AccuraKycModule.faceCL.invoke(null, results.toString());
                    this.finish();
                    return;
                }
                return;
        ...
        if (face2 != null) {
            ...
            try {
                if (!isLiveness) {
                    ...
                    if (!witFace) {
                        results.put("img_1", AccuraKycModule.getImageUri(detectFace1, "img_1", fileDir));
                        results.put("img_2", AccuraKycModule.getImageUri(detectFace2, "img_2", fileDir));
                    } else {
                        results.put("detect", AccuraKycModule.getImageUri(detectFace2, "img_1", fileDir));
                    }
              ...
              AccuraKycModule.faceCL.invoke("Error found in data. Please try again", null);
                e.printStackTrace();
                this.finish();
            }
            this.finish();
